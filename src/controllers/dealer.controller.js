...
const { dealerService, customerService, userService, userRoleService } = require('../services');

const createDealer = catchAsync(async (req, res) => {
  const dealer = await dealerService.createDealer(req.body);
  res.json(new ApiResponse(httpStatus.CREATED, message.SUCCESS, { dealer: dealer }));
});
...
const getDealers = catchAsync(async (req, res) => {
  ...
  res.json(new ApiResponse(httpStatus.OK, message.SUCCESS, { dealers: dealers }));
});
...
const getDealerCustomers = catchAsync(async (req, res) => {
  ...
  const customers = await customerService.getCustomerByDealerId(req.params.dealerId);
  res.json(new ApiResponse(httpStatus.OK, message.SUCCESS, { dealer: dealer, customers: customers}));
});

const updateDealer = catchAsync(async (req, res) => {
  const dealer = await dealerService.updateDealerById(req.params.dealerId, req.body);
  res.json(new ApiResponse(httpStatus.ACCEPTED, message.SUCCESS, { dealer: dealer }));
});
...
const addDealerUser = catchAsync(async (req, res) => {
  ...
  const userRole = await userRoleService.createUserRole({userId: user.id, roleId: dealer.id, role: 'dealer'});
  res.json(new ApiResponse(httpStatus.OK, message.SUCCESS, { dealer: dealer, user: user, userRole: userRole}));
});
...
const getDealerUsers = catchAsync(async (req, res) => {
  ...
  res.json(new ApiResponse(httpStatus.OK, message.SUCCESS, { dealer: dealer, users: users}));
});
...
const deleteDealerUser = catchAsync(async (req, res) => {
  ...
  res.json(new ApiResponse(httpStatus.OK, message.SUCCESS, { dealer: dealer, user: user}));
});
